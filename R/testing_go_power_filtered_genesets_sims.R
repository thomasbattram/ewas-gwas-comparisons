...
## check pathway sizes
all_pathsizes <- get_pathway_size(unique(go_terms$pathway_id), go_terms)
summary(all_pathsizes)

## remove very large pathways (above 1000) and pathways that are just 1 gene...
filtered_pathsizes <- all_pathsizes %>%
	dplyr::filter(genes_in_pathway < 5000) %>%
	dplyr::filter(genes_in_pathway > 1)

go_genes <- go_genes[names(go_genes) %in% filtered_pathsizes$pathway_id]
go_terms <- go_terms %>%
	dplyr::filter(pathway_id %in% filtered_pathsizes$pathway_id)
...
nam <- paste0("geneset_filtering_tests/filter_sim_", split, ".txt")
