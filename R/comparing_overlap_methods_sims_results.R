...
sim_res_path <- "data/derived/sim_temp"
...
### FOR CHANGING BAD FILENAMES!!!
# all_files <- list.files(sim_res_path)
# oldfilenames <- paste0("derivedsim2_kegg_stringdb_proteins", 1:100, ".txt")
# oldfilenames <- oldfilenames[oldfilenames %in% all_files]

# fnam <- oldfilenames[1]
# lapply(oldfilenames, function(fnam) {
# 	sim_number <- gsub("derivedsim2_kegg_stringdb_proteins", "", fnam)
# 	sim_number <- gsub(".txt", "", sim_number)
# 	new_nam <- paste0("sim2_kegg_stringdb_proteins_", sim_number, ".txt")
# 	cmd <- paste("mv", file.path(sim_res_path, fnam), file.path(sim_res_path, new_nam))
# 	print(cmd)
# 	system(cmd)
# })

data_nams <- "go_proteins"
# data_nams <- "go_and_kegg"

bind_results(1, sim_res_path, res_dir = "results/sim")

### ALL SIM2 DATA EXPLAINED:
## sim2_${sim_n}.txt == sim2 with go, kegg, stringdb, using all genes
## epigraph-db_sim2_${sim_n} == sim2 with reactome, ppi, using protein coding genes
## sim2_go_proteins_${sim_n} == sim2 with go, using protein coding genes
## sim2_kegg_stringdb_proteins_${sim_n} == sim2 with kegg, stringdb, using protein coding genes

### What should ultimately be the results:
## sim2_go_kegg_genes_${sim_n}.txt == go, kegg, using all genes
## sim2_go_kegg_stringdb_reactome_ppi_proteins_${sim_n}.txt == all databases, using proteins

# Bind all sim2 data
bind_results(2, sim_res_path, res_dir = "results/sim", 
			 sim_nam = "sim2")
bind_results(2, sim_res_path, res_dir = "results/sim", 
			 sim_nam = "epigraph-db_sim2")
bind_results(2, sim_res_path, res_dir = "results/sim", 
			 sim_nam = "sim2_go_proteins")
bind_results(2, sim_res_path, res_dir = "results/sim", 
			 sim_nam = "sim2_kegg_stringdb_proteins")

# -------------------------------------------------------
# read in results
# -------------------------------------------------------

## removed simulation 1 results as the "pathway down" method of
## extracting genes did not work 

### SIM2
sim2_res_go_kegg_sdb_genes <- read_tsv("results/sim/sim2.txt", guess_max = 1e6)
sim2_res_epidb <- read_tsv("results/sim/epigraph-db_sim2.txt", guess_max = 1e6)
sim2_res_go_proteins <- read_tsv("results/sim/sim2_go_proteins.txt", guess_max = 1e6)
sim2_res_kegg_sdb_proteins <- read_tsv("results/sim/sim2_kegg_stringdb_proteins.txt", guess_max = 1e6)

sim2_res_genes <- sim2_res_go_kegg_sdb_genes %>%
	dplyr::filter(pathway_type != "stringdb")
sim2_res_proteins <- bind_rows(list(sim2_res_epidb, sim2_res_go_proteins, sim2_res_kegg_sdb_proteins)) %>%
	dplyr::filter(n_genes != 25000)

table(sim2_res_proteins$pathway_type) # should be all 5
table(sim2_res_genes$pathway_type) # should just be go + kegg
...
gen_auc_res <- function(sim_res, sim_n) 
{
	...
	if (sim_n == 1) {
		parameters <- c("pathway_type", "ewas_power", "prop_consequent_pathways", "method")
	} else if (sim_n == 2) {
...
plot_auc_res <- function(auc_res, percent_ewas_causal, sim_n, title_nam) 
{
	### take auc res from gen_auc_res and plot them
	auc_plot <- ggplot(auc_res, aes(x = as.factor(ewas_power), y = auc_estimate, 
				...
				labs(x = "EWAS power", y = "AUC", title = title_nam) + 
				theme_bw() # + 
	...
	if (sim_n == 1) {
		...
		auc_plot <- auc_plot + 
			facet_grid(n_genes ~ pathway_type, 
...
# quickly check proteins only data
sim2_res <- sim2_res_proteins %>%
	dplyr::filter(pathway_type != "stringdb")

# get just gene overlap from go terms - should all be the same...
unique_pdb <- unique(sim2_res$pathway_type)

sims_across_all <- lapply(unique_pdb, function(x) {
	sim2_res %>%
		dplyr::filter(pathway_type == x) %>%
		pull(sim) %>%
		unique
})

sims_across_all <- Reduce(intersect, sims_across_all)

go_or_g <- sim2_res %>%
	dplyr::filter(pathway_type == "go") %>%
	arrange(sim, percent_ewas_causal) %>%
	dplyr::filter(sim %in% sims_across_all) %>%
	pull(or_g)

sim2_res <- sim2_res %>%
	dplyr::filter(sim %in% sims_across_all) %>%
	arrange(pathway_type, sim, percent_ewas_causal) %>%
	mutate(or_g = rep(go_or_g, times = length(unique_pdb)))
...
n_genes_labs <- paste("Assoc genes:", unique(sim2_res$n_genes))
...
lapply(seq_along(auc_plot_list), function(x) {
	out_nam <- file.path(out_path, paste0("PEC_", names(auc_plot_list)[x], ".pdf"))
	p_out <- auc_plot_list[[x]] + 
		    theme(strip.text.y = element_text(size = 7)) # changing size of labels from facet
	ggsave(out_nam, plot = p_out)
})
