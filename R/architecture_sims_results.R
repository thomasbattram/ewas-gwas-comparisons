...
bad_res <- sim_res %>%
	dplyr::filter(ca_con_overlap == 0.01, trait == "former_versus_never_smoking")

bad_res$rho_p

x <- 0.298
dist <- sim_res %>% 
	dplyr::filter(trait == "former_versus_never_smoking") %>%
	pull(rho_p)

z_test(x, dist)

### PROBLEM NEEDED TO FIX!!

z_test_res <- sim_res %>%
	left_join(trait_res2) %>% 
	mutate(N_KTG = n_gwas_genes + n_ewas_genes, 
		   R_KTG = paste0(n_gene_proportions, ":1")) %>%
	group_by(trait, N_KTG, R_KTG, ca_con_overlap) %>%
...
cor_z_test_res <- z_test_res %>%
	...
	dplyr::select(trait, N_KTG, R_KTG, ca_assoc_overlap = ca_con_overlap,
				  observed_cor = true_cor, expected_cor = mean_sim_cor, 
				  p_diff = p_diff_cor)

write.table(cor_z_test_res, file = "report/report_data/arch_sims_ztest_sig_res.tsv", 
...
plot_sim_dat <- function(sim_dat, colours, trait_dat, trait_nam, two_traits = FALSE)
{
	...
	p <- ggplot(sim_dat) +
		...
		scale_fill_manual(name = bquote("Ratio of causal and associated genes relative to " ~ N[KTG]),
						    values = cols_to_use) +
		scale_colour_manual(name = "",
						  values = c("TRUE" = "grey", "FALSE" = "black")) +
		guides(colour = FALSE) + 
		labs(x = "Proportion of overlap between causal and associated genes",
			 y = "Correlation of geneset enrichment scores") +
		ylim(0, 1) + 
...
names(cols_to_use) <- c("1:1", "2:1", "3:1", "5:1", "10:1", "20:1")

## just crp and former vs. never smoking
...
ggsave("results/plots/architecture_sims_crp_fvns_only_correlation_of_pathway_enrichment_scores.png", plot = outp)
...
outp <- plot_grid(plotlist = pl, ncol = 3, nrow = 4)
